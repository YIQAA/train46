<template>
  <div class="insurance-selection-wrapper">
    <Space direction="vertical" :style="{ width: '100%' }" size="large">
      ...
      <div class="card-row">
        <Card class="card-container">
          ...
          <Form :form="formRef">
            ...
            <FormItem label="投保人手机号" name="policyHolderPhone">
              <Input v-model:value="formState.policyHolderPhone" />
            </FormItem>
          </Form>
        </Card>
        <Card class="card-container">
          ...
          <Form :form="formRef">
            ...
            <FormItem label="被投保人手机号" name="insuredPhone">
              <Input v-model:value="formState.insuredPhone" />
            </FormItem>
          </Form>
        </Card>
      </div>
      ...
      >
        <Spin :spinning="isLoading">
          ...
          <div :style="{ overflow: 'hidden' }">
            <div v-for="item in BANK_LIST" class="bank3" :key="item.value">
      ...
      <!-- 支付中提示模态框 -->
      <Modal
          v-model:visible="isPayingModalVisible"
          title="网上支付提示"
          :footer="null"
          style="top: 30%"
      >
        ...
        >
          <Button type="default" @click="handlePaymentIssue">
            支付遇到问题
          </Button>
          <Button type="primary" @click="handlePaymentSuccess">
            支付完成
          </Button>
        </Space>
      </Modal>
...
import { ID_CARD_TYPE, INSURANCE_TYPE, BANK_LIST } from '@/constants';
import {fetchBuyInsurance} from '@/service';
...
const isPayingModalVisible = ref(false);
...
const handlePay = (channel) => {
  ...
  isPayingModalVisible.value = true;

  fetchBuyInsurance({
    userId:localStorage.getItem('userId'),
    insuranceType: formState.insuranceType,
    amount: formState.amount,
    policyHolderName: formState.policyHolderName,
    policyHolderIdType: formState.policyHolderIdType,
    policyHolderIdCard: formState.policyHolderIdCard,
    policyHolderPhone: formState.policyHolderPhone,
    insuredName: formState.insuredName,
    insuredIdType: formState.insuredIdType,
    insuredIdCard: formState.insuredIdCard,
    insuredPhone: formState.insuredPhone,
  ...
  setTimeout(() => {
    ...
    isPayingModalVisible.value = false;
    handlePaymentSuccess();
  }, 2000);
};

const handlePaymentIssue = () => {
  // 处理支付遇到问题的逻辑，例如跳转到问题反馈页面
  // router.push('/paymentIssue');
};
